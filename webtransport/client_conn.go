...
import (
	...
	"github.com/malivvan/quic/webtransport/httpsfv"
)
...
func (d *Transport) NewClientConn(qconn *quic.Conn) (*ClientConn, error) {
	...
	// Note: this package's ConnectionState only reports whether the PEER advertised
	// support, so the local QUICConfig.EnableDatagrams / EnableStreamResetPartialDelivery
	// settings cannot be verified here. The peer's support is checked when dialing.
	var config Config
	if d.Config != nil {
		config = *d.Config
	}
...
func (c *ClientConn) dial(ctx context.Context, u *url.URL, reqHdr http.Header) (*http.Response, *Session, error) {
	...
	if !state.SupportsStreamResetPartialDelivery {
