...
// Fingerprint returns a normalized summary of the connection-relevant information
// that can be used to identify the peer's QUIC implementation.
// See [Fingerprint] for details.
func (cs ConnectionState) Fingerprint() Fingerprint {
	fp := Fingerprint{
		Version:             cs.Version,
		ClientHello:         cs.ClientHello,
		SrcConnectionID:     cs.SrcConnectionID,
		DestConnectionID:    cs.DestConnectionID,
		InitialPacketSize:   cs.InitialPacketSize,
		GreaseQuicBit:       cs.GreaseQuicBit,
		TransportParameters: make(map[uint64][]byte),
	}
	// Parse the raw transport parameters to capture the parameter IDs and values
	// in wire order. The raw bytes are the source of truth, as they include GREASE
	// and unknown parameters that the parsed struct does not preserve.
	b := cs.PeerTransportParametersRaw
	for len(b) > 0 {
		...
		fp.TransportParameterIDs = append(fp.TransportParameterIDs, id)
		if _, ok := fp.TransportParameters[id]; !ok {
			fp.TransportParameters[id] = append([]byte(nil), b[:valLen]...)
		}
		b = b[valLen:]
	}
	if len(fp.TransportParameterIDs) == 0 && cs.PeerTransportParameters != nil {
		fp.TransportParameterIDs = slices.Clone(cs.PeerTransportParameters.TransportParameterIDs)
		fp.GreaseQuicBit = cs.PeerTransportParameters.GreaseQuicBit
	}
	if len(cs.ClientHello) > 0 {
		fp.CipherSuites, fp.SupportedVersions, fp.Extensions, fp.SNIs, fp.ALPN = parseClientHelloFingerprint(cs.ClientHello)
	}
	return fp
}
