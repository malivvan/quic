...
func TestInitialCryptoStreamClientECHWithoutSNI(t *testing.T) {
	...
	// A ClientHello that contains an ECH extension but no SNI extension.
	// This is what uTLS presets with a GREASE ECH extension (e.g. Chrome)
	// produce over QUIC. Regression test: the scrambling logic used to
	// compute the SNI cut as -1 (== protocol.InvalidByteCount) in this
	// case, which made HasData() report no data forever and stalled the
	// handshake (no Initial packet was ever sent).
	body := cryptobyte.NewBuilder(nil)
	body.AddUint16(0x0303)          // legacy version
	body.AddBytes(make([]byte, 32)) // random
	body.AddUint8(0)                // session ID length
	body.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
		b.AddUint16(0x1301) // TLS_AES_128_GCM_SHA256
	})
	body.AddUint8(1) // compression methods length
	body.AddUint8(0) // compression: none
	body.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
		b.AddUint16(extTypeECH) // GREASE/real ECH extension, no SNI
		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			b.AddBytes(make([]byte, 8))
		})
	})
	raw := body.BytesOrPanic()
	clientHello := []byte{0x01, byte(len(raw) >> 16), byte(len(raw) >> 8), byte(len(raw))}
	clientHello = append(clientHello, raw...)

	_, err := str.Write(clientHello)
	require.NoError(t, err)
	require.True(t, str.HasData())

	// The ClientHello must be emitted as a sequence of CRYPTO frames that
	// reassemble to the original bytes.
	var segments []*wire.CryptoFrame
	for {
		f := str.PopCryptoFrame(protocol.MaxByteCount)
		if f == nil {
			break
	...
	reassembled := make([]byte, 0, len(clientHello))
	for _, f := range segments {
		reassembled = append(reassembled, f.Data...)
	}
	require.Equal(t, clientHello, reassembled)
}
