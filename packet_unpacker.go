...
type packetUnpacker struct {
	...
	// greaseQuicBit indicates that the peer advertised the grease_quic_bit transport
	// parameter (RFC 9287). In that case, packets with the QUIC bit cleared are accepted.
	greaseQuicBit bool
}
...
func (u *packetUnpacker) unpackLongHeaderPacket(opener handshake.LongHeaderOpener, hdr *wire.Header, data []byte) (*wire.ExtendedHeader, []byte, error) {
	...
	// If the reserved bits (or the fixed bit, when greasing was negotiated) are set incorrectly,
	// we still need to continue unpacking. This avoids a timing side-channel, which otherwise
	// might allow an attacker to gain information about the header encryption.
	if parseErr != nil && parseErr != wire.ErrInvalidReservedBits && !(u.greaseQuicBit && errors.Is(parseErr, wire.ErrInvalidFixedBit)) {
	...
	// A greased QUIC bit (RFC 9287) is not an error: the packet is valid and must be processed.
	if u.greaseQuicBit && errors.Is(parseErr, wire.ErrInvalidFixedBit) {
		parseErr = nil
	}
	if parseErr != nil {
		return nil, nil, parseErr
...
// The error is either nil, a wire.ErrInvalidReservedBits, a tolerated wire.ErrInvalidFixedBit
// or of type headerParseError.
func (u *packetUnpacker) unpackLongHeader(hd headerDecryptor, hdr *wire.Header, data []byte) (*wire.ExtendedHeader, error) {
	extHdr, err := unpackLongHeader(hd, hdr, data, u.greaseQuicBit)
	if err != nil && err != wire.ErrInvalidReservedBits && !(u.greaseQuicBit && errors.Is(err, wire.ErrInvalidFixedBit)) {
		return nil, &headerParseError{err: err}
	}
	return extHdr, err
}

func unpackLongHeader(hd headerDecryptor, hdr *wire.Header, data []byte, greaseQuicBit bool) (*wire.ExtendedHeader, error) {
	...
	if parseErr != nil && parseErr != wire.ErrInvalidReservedBits && !(greaseQuicBit && errors.Is(parseErr, wire.ErrInvalidFixedBit)) {
		return nil, parseErr
	}
	// 4. if the packet number is shorter than 4 bytes, replace the remaining bytes with the copy we saved earlier
