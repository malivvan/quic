...
func (t *Transport) doDial(
	...
	if config.Fingerprint != nil {
		// Override the initial packet size, if requested by the fingerprint configuration.
		// Clamp to the valid range, like validateConfig does.
		if config.Fingerprint.InitialPacketSize > 0 {
			config.InitialPacketSize = config.Fingerprint.InitialPacketSize
			if config.InitialPacketSize < protocol.MinInitialPacketSize {
...
func (t *Transport) listen(conn rawConn) {
	for {
		...
		// See https://github.com/malivvan/quic/issues/1737 for details.
...
// applyFingerprintTransportConfig applies the client-side fingerprint configuration to the
// Transport: it sets the connection ID length used for the client's source connection ID.
func applyFingerprintTransportConfig(tr *Transport, conf *Config) {
	if conf == nil || conf.Fingerprint == nil || conf.Fingerprint.ConnectionIDLength <= 0 {
		return
	}
	if tr.ConnectionIDLength != 0 && tr.ConnectionIDLength != conf.Fingerprint.ConnectionIDLength {
		// The user explicitly set a different connection ID length; respect it.
		return
	}
	if l := conf.Fingerprint.ConnectionIDLength; l <= protocol.MaxConnIDLen {
		tr.ConnectionIDLength = l
	}
}
