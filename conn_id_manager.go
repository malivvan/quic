...
type connIDManager struct {
	...
	// connIDLimit is the maximum number of connection IDs that can be stored,
	// i.e. the active_connection_id_limit advertised to the peer.
	// It is at least protocol.MaxActiveConnectionIDs.
	connIDLimit uint32
...
// SetConnIDLimit sets the maximum number of connection IDs that can be stored.
// It must match the active_connection_id_limit advertised to the peer.
func (h *connIDManager) SetConnIDLimit(limit uint64) {
	if limit > protocol.MaxConnIDLen {
		limit = protocol.MaxConnIDLen
	}
	if uint64(protocol.MaxActiveConnectionIDs) < limit {
		h.connIDLimit = uint32(limit)
	}
}
...
func (h *connIDManager) Add(f *wire.NewConnectionIDFrame) error {
	...
	if len(h.queue) >= int(h.connIDLimit) {
...
// See https://github.com/malivvan/quic/pull/4852 for more details.
