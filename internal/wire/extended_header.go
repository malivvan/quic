...
// ErrInvalidFixedBit is returned when the fixed bit (the QUIC bit, RFC 9287) is not set.
// When this error is returned, parsing continues, and the header is returned.
// Peers that advertise the grease_quic_bit transport parameter are allowed to clear
// the QUIC bit, in which case the receiver must ignore it.
var ErrInvalidFixedBit = errors.New("not a QUIC packet")
