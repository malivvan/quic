...
func ParseShortHeader(data []byte, connIDLen int) (length int, _ protocol.PacketNumber, _ protocol.PacketNumberLen, _ protocol.KeyPhaseBit, _ error) {
	...
	// The fixed bit (the QUIC bit, RFC 9287) may be cleared by peers that advertised
	// grease_quic_bit. The error is returned together with the parsed packet,
	// so that the caller can tolerate it in that case.
	var fixedBitErr error
	if data[0]&0x40 == 0 {
		fixedBitErr = ErrInvalidFixedBit
	}
