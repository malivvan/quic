...
func ParsePacket(data []byte) (*Header, []byte, []byte, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUnsupportedVersion) || errors.Is(err, ErrInvalidFixedBit) {
			// ErrInvalidFixedBit (RFC 9287) is returned together with the parsed header,
			// so that the caller can tolerate it when QUIC bit greasing was negotiated.
...
func (h *Header) parseLongHeader(b []byte) (int, error) {
	...
	// Note: the fixed bit (QUIC bit, RFC 9287) is checked at the end of this function,
	// after the header has been fully parsed. This allows the caller to tolerate
	// packets from peers that advertise the grease_quic_bit transport parameter.
	...
	l := startLen - len(b) + n
	// The fixed bit (QUIC bit) must be set, unless the peer advertised grease_quic_bit (RFC 9287).
	// The error is returned together with the fully parsed header, so that the caller
	// can tolerate it when greasing was negotiated.
	if h.Version != 0 && h.typeByte&0x40 == 0 {
		return l, ErrInvalidFixedBit
	}
	return l, nil
}
