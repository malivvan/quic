...
type AckFrame struct {
	...
	// AckDelayExponent is the ack_delay_exponent used to encode DelayTime.
	// 0 means the protocol default (protocol.AckDelayExponent).
	// This field is not reset by Reset, as it is a property of the sender, not of the frame contents.
	AckDelayExponent uint8
}

// encodeAckDelay encodes the ack delay using the frame's ack_delay_exponent.
func (f *AckFrame) encodeAckDelay() uint64 {
	exponent := f.AckDelayExponent
	if exponent == 0 {
		exponent = protocol.AckDelayExponent
	}
	return encodeAckDelayWithExponent(f.DelayTime, exponent)
}
...
func (f *AckFrame) Append(b []byte, _ protocol.Version) ([]byte, error) {
	...
	b = quicvarint.Append(b, f.encodeAckDelay())
...
func (f *AckFrame) Length(_ protocol.Version) protocol.ByteCount {
	...
	length := 1 + quicvarint.Len(uint64(largestAcked)) + quicvarint.Len(f.encodeAckDelay()) + 1
...
func (f *AckFrame) numEncodableAckRanges(maxSize protocol.ByteCount) int {
	...
	length = 1 + quicvarint.Len(uint64(f.LargestAcked())) + quicvarint.Len(f.encodeAckDelay()) + 1
	_, firstRange := f.encodeAckRange(0)
...
// encodeAckDelayWithExponent encodes an ack delay using the given ack_delay_exponent
// (the exponent advertised in the transport parameters).
func encodeAckDelayWithExponent(delay time.Duration, exponent uint8) uint64 {
	return uint64(delay.Nanoseconds() / (1000 * (1 << exponent)))
}
