...
// quicConn is the subset of the tls.QUICConn API used by the crypto setup.
// Both *tls.QUICConn and *tls.UQUICConn (uTLS) implement it.
...
// sessionStorer is implemented by *tls.QUICConn, but not by *tls.UQUICConn (uTLS).
// The uTLS QUIC path does not emit QUICStoreSession events, so storing sessions is a no-op there.
...
type cryptoSetup struct {
	...
	// peerTransportParametersRaw stores the raw transport parameter bytes received from the peer,
	// exactly as they appeared in the TLS handshake, for fingerprinting.
...
// If clientHelloID is set (tls.ClientHelloID.IsSet() returns true), the ClientHello is built
// from the given uTLS preset, mimicking the TLS fingerprint of that client.
// transportParameterOrder and extraTransportParameters are forwarded to
// wire.TransportParameters.MarshalWithConfig: a non-empty order emits the parameters in the
// given order (no random GREASE), with extra appended at the end (see FingerprintConfig).
func NewCryptoSetupClient(
	...
	// QUIC requires TLS 1.3, and the uTLS presets are captured from TCP connections
	// and may contain TLS 1.2 in their supported_versions extension.
	tlsConf.MaxVersion = tls.VersionTLS13
	cs.tlsConf = tlsConf
	cs.allow0RTT = enable0RTT

	// The QUIC transport parameters are marshaled once here, and sent via SetTransportParameters.
	// When a uTLS ClientHelloID is used, the transport parameters are additionally injected
	// into the ClientHello's quic_transport_parameters extension (see newUQUICClientConn).
	tpBytes := cs.ourParams.MarshalWithConfig(protocol.PerspectiveClient, transportParameterOrder, extraTransportParameters)
	if clientHelloID.Client == "" && clientHelloID.Version == "" {
		// No uTLS preset: use the standard QUICConn.
		cs.conn = tls.QUICClient(&tls.QUICConfig{
			TLSConfig:           tlsConf,
			EnableSessionEvents: true,
		})
	} else {
		cs.conn = newUQUICClientConn(&tls.QUICConfig{TLSConfig: tlsConf}, clientHelloID, tpBytes)
		// Note: the uTLS QUIC path does not support session events (and thus 0-RTT resumption).
	}
	cs.conn.SetTransportParameters(tpBytes)
...
func (h *cryptoSetup) handleEvent(ev tls.QUICEvent) (err error) {
	...
	case tls.QUICStoreSession:
		...
		// The uTLS QUIC path (*tls.UQUICConn) does not emit QUICStoreSession events,
		// so this is only ever called for *tls.QUICConn.
		if s, ok := h.conn.(sessionStorer); ok {
			return s.StoreSession(ev.SessionState)
		}
		return nil
...
func (h *cryptoSetup) handleTransportParameters(data []byte) error {
	// Store the raw bytes for fingerprinting before unmarshaling.
	// The TLS stack reuses its internal buffers, so we need to copy.
	h.peerTransportParametersRaw = slices.Clone(data)
...
func (h *cryptoSetup) accumulateClientHello(data []byte) {
	if len(data) == 0 || h.clientHelloDone {
		return
	}
	...
	// The first byte of the first handshake message at the Initial level is the handshake
	// message type. If it's not a ClientHello, there's nothing to accumulate.
	if h.clientHelloLen < 0 && h.clientHelloBuf[0] != 0x01 {
...
func (h *cryptoSetup) ConnectionState() ConnectionState {
	return ConnectionState{
		...
		PeerTransportParameters:    h.peerParams,
		PeerTransportParametersRaw: h.peerTransportParametersRaw,
	}
}
