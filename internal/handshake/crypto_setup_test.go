...
func TestServerCapturesClientHelloAndTransportParameters(t *testing.T) {
	clientConf, serverConf := getTLSConfigs()
	clientParams := &wire.TransportParameters{ActiveConnectionIDLimit: 2, GreaseQuicBit: true}
	serverParams := &wire.TransportParameters{ActiveConnectionIDLimit: 2}
	_, _, clientErr, server, _, serverErr := handshakeWithTLSConf(
		t,
		clientConf, serverConf,
		utils.NewRTTStats(), utils.NewRTTStats(),
	...
	// The raw ClientHello must have been captured, starting with the handshake type byte.
	require.NotEmpty(t, cs.ClientHello)
	require.Equal(t, byte(1), cs.ClientHello[0]) // handshakeTypeClientHello
	// The raw transport parameters must have been captured.
	require.NotEmpty(t, cs.PeerTransportParametersRaw)
	// The parsed transport parameters must be available, including the grease_quic_bit.
	require.NotNil(t, cs.PeerTransportParameters)
	require.True(t, cs.PeerTransportParameters.GreaseQuicBit)
	require.Contains(t, cs.PeerTransportParameters.TransportParameterIDs, uint64(0x2ab2))
}

func TestClientHelloCaptureWithFragmentedClientHello(t *testing.T) {
	clientConf, serverConf := getTLSConfigs()
	client := NewCryptoSetupClient(
		protocol.ConnectionID{},
		&wire.TransportParameters{},
	...
	var ch []byte
	// Feed the client's Initial flight to the server in 1-byte fragments,
	// so the ClientHello accumulation is exercised across fragments.
	for {
		ev := client.NextEvent()
		if ev.Kind == EventNoEvent {
			break
		}
		if ev.Kind == EventWriteInitialData {
			ch = ev.Data
		}
	}
	require.NotEmpty(t, ch)
	// Deliver the ClientHello record in fragments of 7 bytes.
	for i := 0; i < len(ch); i += 7 {
		end := min(i+7, len(ch))
		require.NoError(t, server.HandleMessage(ch[i:end], protocol.EncryptionInitial))
	}
	cs := server.ConnectionState()
	require.NotEmpty(t, cs.ClientHello)
	require.Equal(t, byte(1), cs.ClientHello[0])
	require.Equal(t, ch[:len(cs.ClientHello)], cs.ClientHello)
}

func TestClientHelloCaptureStopsAfterFirstMessage(t *testing.T) {
	// A non-ClientHello first message at the Initial level must terminate accumulation.
	cs := &cryptoSetup{clientHelloLen: -1}
	cs.accumulateClientHello([]byte{0x02, 0x00, 0x00, 0x06, 'f', 'o', 'o'})
	require.True(t, cs.clientHelloDone)
	require.Nil(t, cs.clientHello)
	require.Nil(t, cs.clientHelloBuf)
}

func TestClientHelloCaptureEmptyClientHello(t *testing.T) {
	// A ClientHello with a zero-length body must be captured and marked done.
	cs := &cryptoSetup{clientHelloLen: -1}
	cs.accumulateClientHello([]byte{0x01, 0x00, 0x00, 0x00})
	require.True(t, cs.clientHelloDone)
	require.Equal(t, []byte{0x01, 0x00, 0x00, 0x00}, cs.clientHello)
}

func TestUTLSClientHandshake(t *testing.T) {
	// A client using a uTLS ClientHelloID preset must complete a handshake with a
	// standard server, and the ClientHello must contain the quic_transport_parameters
	// extension (0x0039) with our transport parameters.
	clientConf, serverConf := getTLSConfigs()
	client := NewCryptoSetupClient(
		protocol.ConnectionID{},
		&wire.TransportParameters{ActiveConnectionIDLimit: 2, InitialSourceConnectionID: protocol.ParseConnectionID([]byte{1, 2, 3, 4, 5, 6, 7, 8})},
		clientConf,
		false,
		utils.NewRTTStats(),
		nil,
	...
	require.NoError(t, clientErr)
	require.NoError(t, serverErr)
	require.NotEmpty(t, clientEvents)
	require.NotEmpty(t, serverEvents)

	// The server must have received the ClientHello, including the transport parameters.
	cs := server.ConnectionState()
	require.NotEmpty(t, cs.ClientHello)
	require.True(t, bytes.Contains(cs.ClientHello, []byte{0x00, 0x39})) // quic_transport_parameters extension
	require.NotEmpty(t, cs.PeerTransportParametersRaw)
	require.Equal(t, protocol.ParseConnectionID([]byte{1, 2, 3, 4, 5, 6, 7, 8}), cs.PeerTransportParameters.InitialSourceConnectionID)
}
