...
import (
	...
	"github.com/malivvan/quic/internal/ackhandler"
	"github.com/malivvan/quic/internal/handshake"
	"github.com/malivvan/quic/internal/monotime"
	"github.com/malivvan/quic/internal/protocol"
	"github.com/malivvan/quic/internal/qerr"
	"github.com/malivvan/quic/internal/utils"
	"github.com/malivvan/quic/internal/utils/ringbuffer"
	"github.com/malivvan/quic/internal/wire"
	"github.com/malivvan/quic/qlog"
	"github.com/malivvan/quic/qlogwriter"
)
...
type Conn struct {
	...
	// srcConnID is the client's own source connection ID (only set for the client).
...
var newConnection = func(
	...
	clientSrcConnID protocol.ConnectionID,
	...
	s.preSetup()
	// The source connection ID of the client's first Initial packet, for fingerprinting.
	s.connState.SrcConnectionID = clientSrcConnID
	...
	params := &wire.TransportParameters{
		...
		// See https://github.com/malivvan/quic/pull/3806.
		ActiveConnectionIDLimit:   protocol.MaxActiveConnectionIDs,
		InitialSourceConnectionID: srcConnID,
		RetrySourceConnectionID:   retrySrcConnID,
		EnableResetStreamAt:       conf.EnableStreamResetPartialDelivery,
		GreaseQuicBit:             conf.GreaseQuicBit,
	}
	s.connIDManager.SetConnIDLimit(params.ActiveConnectionIDLimit)
...
var newClientConnection = func(
	...
	// Apply the client-side fingerprint configuration (transport parameter overrides, grease_quic_bit).
	s.config.Fingerprint.applyToParams(params)
	s.connIDManager.SetConnIDLimit(params.ActiveConnectionIDLimit)
	if s.qlogger != nil {
		s.qlogTransportParameters(params, protocol.PerspectiveClient, false)
	}
	var clientHelloID tls.ClientHelloID
	var transportParameterOrder []uint64
	var extraTransportParameters map[uint64][]byte
	if s.config.Fingerprint != nil {
		clientHelloID = s.config.Fingerprint.ClientHelloID
		transportParameterOrder = s.config.Fingerprint.TransportParameterOrder
		extraTransportParameters = s.config.Fingerprint.ExtraTransportParameters
	}
	cs := handshake.NewCryptoSetupClient(
		...
		s.version,
		clientHelloID,
		transportParameterOrder,
		extraTransportParameters,
	)
	s.cryptoStreamHandler = cs
	...
	s.packer = newPacketPacker(srcConnID, s.connIDManager.Get, s.initialStream, s.handshakeStream, s.sentPacketHandler, s.retransmissionQueue, cs, s.framer, &s.receivedPacketHandler, s.datagramQueue, s.perspective)
	s.setGreaseQuicBit(false)
	// Use the configured ack_delay_exponent (if any) for encoding ACK frames.
	if exp := s.config.Fingerprint.ackDelayExponent(); exp != 0 {
		s.packer.(*packetPacker).ackDelayExponent = exp
	}
...
func (c *Conn) preSetup() {
	...
	c.connState.InitialPacketSize = int(c.config.InitialPacketSize)
	if c.perspective == protocol.PerspectiveClient {
		c.connState.SrcConnectionID = c.srcConnID
		c.connState.DestConnectionID = c.origDestConnID
	} else {
		// SrcConnectionID and DestConnectionID are set by the server (from the client's first
		// Initial packet) right after the connection is created.
		c.connState.DestConnectionID = c.origDestConnID
	}
}
...
func (c *Conn) ConnectionState() ConnectionState {
	...
	c.connState.ClientHello = cs.ClientHello
	c.connState.PeerTransportParameters = cs.PeerTransportParameters
	c.connState.PeerTransportParametersRaw = cs.PeerTransportParametersRaw
	if c.peerParams != nil {
		c.connState.GreaseQuicBit = c.peerParams.GreaseQuicBit
	}
	return c.connState
}
...
func (c *Conn) restoreTransportParameters(params *wire.TransportParameters) {
	...
	c.setGreaseQuicBit(params.GreaseQuicBit)
	c.connStateMutex.Lock()
	c.connState.SupportsDatagrams = c.supportsDatagrams()
	c.connStateMutex.Unlock()
}

// setGreaseQuicBit updates the QUIC bit (RFC 9287) handling based on whether the peer
// advertised the grease_quic_bit transport parameter:
//   - the unpacker tolerates packets with the QUIC bit cleared if we advertised
//     grease_quic_bit ourselves, or the peer did (RFC 9287 Section 3.2),
//   - the packer sets the QUIC bit to a random value on ~50% of the packets if both
//     endpoints advertised grease_quic_bit (mimicking the behavior of real browsers).
func (c *Conn) setGreaseQuicBit(peerGrease bool) {
	advertised := c.advertiseGreaseQuicBit()
	if u, ok := c.unpacker.(*packetUnpacker); ok {
		u.greaseQuicBit = advertised || peerGrease
	}
	if p, ok := c.packer.(*packetPacker); ok {
		p.greaseQuicBit = advertised && peerGrease
	}
}

// advertiseGreaseQuicBit returns whether this endpoint advertises the grease_quic_bit
// transport parameter (RFC 9287): either via Config.GreaseQuicBit, or via the client-side
// fingerprint configuration.
func (c *Conn) advertiseGreaseQuicBit() bool {
	if c.config.GreaseQuicBit {
		return true
	}
	return c.perspective == protocol.PerspectiveClient && c.config.Fingerprint != nil && c.config.Fingerprint.GreaseQuicBit
}
