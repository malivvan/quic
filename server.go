...
type baseServer struct {
	...
	newConn func(
		...
		*protocol.ConnectionID, /* retry src connection ID */
		...
		protocol.ConnectionID, /* client source connection ID */
...
func (s *baseServer) handleInitialImpl(p receivedPacket, hdr *wire.Header) error {
	...
	clientInfo := &ClientInfo{
		...
		Version:           hdr.Version,
		InitialPacketSize: int(p.Size()),
		SrcConnectionID:   hdr.SrcConnectionID,
		DestConnectionID:  origDestConnID,
		// Best-effort extraction of the ClientHello from the first Initial packet.
		// The complete ClientHello is available via ConnectionState.ClientHello.
		ClientHello: extractClientHelloFromInitial(p.data, hdr),
	}
	...
	conn = s.newConn(
		...
		hdr.SrcConnectionID, // client source connection ID (for fingerprinting)
		hdr.SrcConnectionID,
		connID,
		s.connIDGenerator,
		s.statelessResetter,
...
func (s *baseServer) maybeSendInvalidToken(p rejectedPacket) {
	...
	extHdr, err := unpackLongHeader(opener, hdr, data, false)
	// Only send INVALID_TOKEN if we can unprotect the packet.
	// This makes sure that we won't send it for packets that were corrupted.
	if err != nil {
		if s.qlogger != nil {
