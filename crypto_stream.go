...
func (s *initialCryptoStream) Write(p []byte) (int, error) {
	...
	if s.cuts[0].start == protocol.InvalidByteCount {
		...
		if sniPos == -1 {
			// ECH extension found, but no SNI (e.g. a uTLS preset with a
			// GREASE ECH extension). There is no SNI to scramble, so only
			// the ECH extension is cut. Without this special case, the SNI
			// cut would be computed as -1, which equals
			// protocol.InvalidByteCount and would make HasData() report no
			// data forever, stalling the handshake.
			start := protocol.ByteCount(echPos + 1)
			s.cuts[0].start = start
			// cut somewhere (16 bytes), most likely in the ECH extension value
			s.cuts[0].end = min(start+16, s.end)
			return len(p), nil
		}
