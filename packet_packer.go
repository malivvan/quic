...
type packetPacker struct {
	...
	// ackDelayExponent is the exponent used for encoding ACK frame delays.
	// 0 means the protocol default (protocol.AckDelayExponent).
	ackDelayExponent uint8
	// greaseQuicBit indicates that the QUIC bit (RFC 9287) should be set to a random value
	// on ~50% of the packets sent, mimicking the behavior of real browsers.
	// It is only enabled when both endpoints advertised grease_quic_bit.
	greaseQuicBit bool
...
// getAckFrame returns the next ACK frame to send, setting the ack_delay_exponent
// used for encoding the ack delay.
func (p *packetPacker) getAckFrame(encLevel protocol.EncryptionLevel, now monotime.Time, onlyIfQueued bool) *wire.AckFrame {
	ack := p.acks.GetAckFrame(encLevel, now, onlyIfQueued)
	if ack != nil {
		ack.AckDelayExponent = p.ackDelayExponent
	}
	return ack
}

// maybeGreaseQuicBit randomly clears the QUIC bit (RFC 9287) of the packet, mimicking
// the behavior of real browsers. The QUIC bit is not covered by header protection,
// so it can be modified after the packet has been encrypted.
func (p *packetPacker) maybeGreaseQuicBit(data []byte, startLen int) {
...
func (p *packetPacker) maybeGetCryptoPacket(
	...
	if onlyAck {
		if ack := p.getAckFrame(encLevel, now, true); ack != nil {
			hdr := p.getLongHeader(encLevel, v)
	...
	ack := p.getAckFrame(encLevel, now, !hasRetransmission && !hasCryptoData())
	var pl payload
...
func (p *packetPacker) composeNextPacket(
	...
	if onlyAck {
		if ack := p.getAckFrame(protocol.Encryption1RTT, now, true); ack != nil {
	...
	var pl payload
	if ackAllowed {
		if ack := p.getAckFrame(protocol.Encryption1RTT, now, !hasRetransmission && !hasData); ack != nil {
			ack.Truncate(maxPayloadSize, v)
			pl.ack = ack
			pl.length += ack.Length(v)
		}
...
func (p *packetPacker) appendShortHeaderPacket(
	...
	buffer.Data = buffer.Data[:len(buffer.Data)+len(raw)]
	p.maybeGreaseQuicBit(buffer.Data, startLen)
