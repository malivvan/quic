...
func testParserSettingsFrameDatagram(t *testing.T, enabled bool) {
	...
	require.NoError(t, err)
	if !enabled {
		// if datagram is disabled, it should be treated as if the setting was absent
		sf.RawSettings = nil
	}
	require.Equal(t, sf, f2)
}
...
func testParserSettingsFrameExtendedConnect(t *testing.T, enabled bool) {
	...
	require.NoError(t, err)
	if !enabled {
		// if extended connect is disabled, it should be treated as if the setting was absent
		sf.RawSettings = nil
	}
	require.Equal(t, sf, f2)
}
