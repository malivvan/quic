...
// RawConnContextKey is a context key for accessing the underlying HTTP/3 connection
// from a request handler. The associated value implements the [RawConn] interface.
// This is useful for TLS/HTTP fingerprinting (e.g. the raw SETTINGS and the QUIC
// connection state).
var RawConnContextKey = &contextKey{"raw-conn"}

// RawConn is the subset of the HTTP/3 connection API exposed to request handlers
// via the [RawConnContextKey] context value.
type RawConn interface {
	// ConnectionState returns the QUIC connection state, including the fingerprinting fields.
	ConnectionState() quic.ConnectionState
	// Settings returns the settings received from the peer.
	Settings() *Settings
