...
import (
	...
	"github.com/malivvan/http/httptrace"

	"github.com/malivvan/quic"
	"github.com/malivvan/quic/http3/qlog"
	"github.com/malivvan/quic/qlogwriter"
	"github.com/malivvan/quic/qpack"
...
type ClientConn struct {
	...
	// maxFieldSectionSizeSetting is the value of SETTINGS_MAX_FIELD_SECTION_SIZE
	// sent to the peer. It is kept separate from maxResponseHeaderBytes: a
	// negative value means the setting is not sent at all (Firefox behavior),
	// while the receiving limit still applies.
	maxFieldSectionSizeSetting int64
...
func newClientConn(
	...
	if maxResponseHeaderBytes < 0 {
		...
		c.maxFieldSectionSizeSetting = -1
	} else if maxResponseHeaderBytes == 0 {
		// Zero means use default
		c.maxResponseHeaderBytes = defaultMaxResponseHeaderBytes
		c.maxFieldSectionSizeSetting = int64(c.maxResponseHeaderBytes)
	} else {
		c.maxResponseHeaderBytes = maxResponseHeaderBytes
		c.maxFieldSectionSizeSetting = int64(maxResponseHeaderBytes)
	}
	...
	go func() {
		str, err := c.rawConn.openControlStream(&settingsFrame{
			...
			MaxFieldSectionSize: c.maxFieldSectionSizeSetting,
		})
		if err != nil {
			if c.logger != nil {
		...
		// Chrome sends priority information on the control stream, before GREASE frames.
		if c.priorityParam > 0 {
			if _, err := str.Write(appendPriorityUpdateFrame(nil, c.priorityParam)); err != nil {
				c.conn.CloseWithError(quic.ApplicationErrorCode(ErrCodeInternalError), "")
				return
			}
		}
		...
		if c.sendGreaseFrames {
			if _, err := str.Write(appendGreaseFrame(nil)); err != nil {
				c.conn.CloseWithError(quic.ApplicationErrorCode(ErrCodeInternalError), "")
				return
			}
		}
...
// The frame type is the priority parameter (e.g. 984832 = 0xF0700 for Chrome).
func appendPriorityUpdateFrame(b []byte, priorityParam uint32) []byte {
	// Frame type is given by the priority parameter.
