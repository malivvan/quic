...
func (f *settingsFrame) Append(b []byte) []byte {
	...
	// in a deterministic order: the built-in settings first (MAX_FIELD_SECTION_SIZE,
	// DATAGRAM, EXTENDED_CONNECT), then the custom settings sorted by ID. A random
	// map-iteration order here would make the fingerprint non-deterministic.
	if val, ok := allSettings[settingMaxFieldSectionSize]; ok {
		b = quicvarint.Append(b, settingMaxFieldSectionSize)
		b = quicvarint.Append(b, val)
		delete(allSettings, settingMaxFieldSectionSize)
	}
	if val, ok := allSettings[settingDatagram]; ok {
		b = quicvarint.Append(b, settingDatagram)
		b = quicvarint.Append(b, val)
		delete(allSettings, settingDatagram)
	}
	if val, ok := allSettings[settingExtendedConnect]; ok {
		b = quicvarint.Append(b, settingExtendedConnect)
		b = quicvarint.Append(b, val)
		delete(allSettings, settingExtendedConnect)
	}
	remaining := make([]uint64, 0, len(allSettings))
	...
	for _, id := range remaining {
		b = quicvarint.Append(b, id)
		b = quicvarint.Append(b, allSettings[id])
	}
